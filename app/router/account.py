...
from services.token import (
    create_tokens,
    get_access_token_account,
    check_refresh_token,
    create_access_token,
)

router = APIRouter(
    prefix="/account", tags=["account"], responses={404: {"description": "Not found"}}
)
...
async def register(regsiter_account: CreateAccount, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_4ti01_UNAUTHORIZED, detail="계정이 생성되지 않았습니다."
        )
...
def login(login_account: LoginAccount, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="로그인 도중 문제가 발생했습니다."
        )
    return output_token
...
async def refresh_token(token: InputRefreshToken, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="토큰 발생 중 문제가 발생했습니다."
        )
...
async def logout(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="로그아웃 도중 문제가 발생했습니다."
        )
