...
from services.money_keep import (
    create_financial_ledge,
    get_financial_ledges,
    get_financial_ledge,
    update_financial_ledge,
    remove_money_keep,
    copy_money_keep_template,
)
from services.token import get_access_token_account

router = APIRouter(tags=["money_keep"], responses={404: {"description": "Not found"}})


@router.post("/money-keep", status_code=200)
def create_money_keep(
    money_keep_data: CreateFinancialLedge,
    account_id: UUID = Depends(get_access_token_account),
...
def read_money_keeps(
    account_id: UUID = Depends(get_access_token_account), db: Session = Depends(get_db)
):
    try:
        db_account = get_account(db, account_id)
        if db_account is None:
...
def update_money_keep(
    ...
    try:
        ...
        if db_account is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
        db_financial_ledge = update_financial_ledge(
            db, financial_ledge_id, money_keep_data, account_id
        )
    except SQLAlchemyError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
    return db_financial_ledge
